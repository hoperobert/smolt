...
**Key capabilities:**
- **Custom tools** - Register tools the LLM can call via `smolt.registerTool()`
- **Event interception** - Block or modify tool calls, inject context, customize compaction
- **User interaction** - Prompt users via `ctx.ui` (select, confirm, input, notify)
- **Custom UI components** - Full TUI components with keyboard input via `ctx.ui.custom()` for complex interactions
- **Custom commands** - Register commands like `/mycommand` via `smolt.registerCommand()`
- **Session persistence** - Store state that survives restarts via `smolt.appendEntry()`
...
Create `~/.smolt/agent/extensions/my-extension.ts`:
...
Extensions are auto-discovered from trusted locations. Project-local `.smolt/extensions` entries load only after the project is trusted.

| Location | Scope |
|----------|-------|
| `~/.smolt/agent/extensions/*.ts` | Global (all projects) |
| `~/.smolt/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.smolt/extensions/*.ts` | Project-local |
| `.smolt/extensions/*/index.ts` | Project-local (subdirectory) |
...
| Package | Purpose |
|---------|---------|
| `smolt` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@smolt/ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@smolt/tui` | TUI components for custom rendering |
...
export default function (smolt: ExtensionAPI) {
  // Subscribe to events
  smolt.on("event_name", async (event, ctx) => {
  ...
  smolt.registerTool({ ... });
  smolt.registerCommand("name", { ... });
  smolt.registerShortcut("ctrl+x", { ... });
  smolt.registerFlag("my-flag", { ... });
}
```
...
export default async function (smolt: ExtensionAPI) {
  ...
  smolt.registerProvider("local-openai", {
...
This pattern makes the fetched models available during normal startup and to `smolt --list-models`.
...
thinking level changes (settings, keybinding, smolt.setThinkingLevel())
...
```typescript
smolt.on("resources_discover", async (event, _ctx) => {
  // event.cwd - current working directory
...
```typescript
smolt.on("session_start", async (event, ctx) => {
...
Fired when the current session display name is set via `/name`, RPC, or `smolt.setSessionName()`.

```typescript
smolt.on("session_info_changed", async (event, ctx) => {
...
```typescript
smolt.on("session_before_switch", async (event, ctx) => {
...
smolt.on("session_compact", async (event, ctx) => {
...
smolt.on("session_compact_failed", async (event, ctx) => {
  // event.reason - "manual" (/compact), "threshold", or "overflow"
...
```typescript
smolt.on("session_before_tree", async (event, ctx) => {
...
smolt.on("session_tree", async (event, ctx) => {
...
```typescript
smolt.on("session_shutdown", async (event, ctx) => {
...
The `systemPromptOptions` field gives extensions access to the same structured data Smolt uses to build the system prompt. This lets you inspect what Smolt has loaded — custom prompts, guidelines, tool snippets, context files, skills — without re-discovering resources or re-parsing flags. Use it when your extension needs to make deep, informed changes to the system prompt while respecting user-provided configuration.

Inside `before_agent_start`, `event.systemPrompt` and `ctx.getSystemPrompt()` both reflect the chained system prompt as of the current handler. Later `before_agent_start` handlers can still modify it again.

#### agent_start / agent_end / agent_settled

`agent_start` fires when a low-level agent run begins. `agent_end` fires when that run ends, but Smolt may still auto-retry, auto-compact and retry, or continue with queued follow-up messages. Use `agent_settled` for status integrations that need to know Smolt will not continue running automatically.
...
```typescript
smolt.on("ui_prompt_start", async (event, ctx) => {
  // event.reason === "ui_prompt"
  // event.kind: "select" | "confirm" | "input" | "editor" | "custom"
  // event.title: prompt title when available
});

smolt.on("ui_prompt_end", async (event, ctx) => {
  // Smolt is no longer waiting on that UI prompt span.
});
```
...
```typescript
smolt.on("turn_start", async (event, ctx) => {
...
```typescript
smolt.on("message_start", async (event, ctx) => {
  // event.message
});

smolt.on("message_update", async (event, ctx) => {
  // event.message
...
```typescript
smolt.on("tool_execution_start", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args
});

smolt.on("tool_execution_update", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args, event.partialResult
});

smolt.on("tool_execution_end", async (event, ctx) => {
...
```typescript
smolt.on("context", async (event, ctx) => {
...
```typescript
smolt.on("before_provider_headers", (event, ctx) => {
  // Add or override — e.g. a session id for gateway tracing/attribution
  event.headers["x-session-id"] = ctx.sessionManager.getSessionId();

  // Drop a tracking header smolt adds for this call
...
This hook can rewrite provider-level system instructions or remove them entirely. Those payload-level changes are not reflected by `ctx.getSystemPrompt()`, which reports Smolt's system prompt string rather than the final serialized provider payload.
...
```typescript
smolt.on("thinking_level_select", async (event, ctx) => {
...
Use this to update extension UI when `smolt.setThinkingLevel()`, model changes, or built-in thinking-level controls change the active thinking level.
...
```typescript
import { isToolCallEventType } from "smolt";
...
```typescript
import { isBashToolResult } from "smolt";
...
```typescript
import { createLocalBashOperations } from "smolt";

smolt.on("user_bash", (event, ctx) => {
  ...
  // Option 2: Wrap smolt's built-in local bash backend
...
Use `CONFIG_DIR_NAME` instead of hardcoding `.smolt` when constructing project-local config paths. Rebranded distributions can use a different config directory name.

```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "smolt";
...
Access to models, providers, and resolved authentication. `ctx.modelRegistry.getProvider(id)` returns the effective smolt-ai provider, while `getProviderAuth(id)` resolves its current API key, headers, base URL, and provider-scoped environment without requiring a loaded model. `ctx.model` is the active model, and `ctx.thinkingLevel` is its current effective thinking level.
...
Control flow helpers. `ctx.isIdle()` is false while Smolt is processing an agent run, automatic retry, auto-compaction retry, or queued continuation.
...
Returns Smolt's current system prompt string.
...
```typescript
smolt.on("before_agent_start", (event, ctx) => {
...
Returns the base inputs Smolt currently uses to build the system prompt.
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `smolt` / command `ctx`; see [Session replacement lifecycle and footguns](#session-replacement-lifecycle-and-footguns).
...
```typescript
import { SessionManager } from "smolt";

smolt.registerCommand("switch", {
...
- Captured old `smolt` / old command `ctx` session-bound objects are stale after replacement and will throw if used. Use only the `ctx` passed to `withSession` for session-bound work.
...
```typescript
smolt.registerCommand("reload-runtime", {
...
export default function (smolt: ExtensionAPI) {
  ...
  smolt.registerTool({
    ...
    parameters: Type.Object({}),
    async execute() {
      smolt.sendUserMessage("/reload-runtime", { deliverAs: "followUp" });
      return {
...
`smolt.registerTool()` works both during extension load and after startup. You can call it inside `session_start`, command handlers, or other event handlers. New tools are refreshed immediately in the same session, so they appear in `smolt.getAllTools()` and are callable by the LLM without `/reload`.

Use `smolt.setActiveTools()` to enable or disable tools (including dynamically added tools) at runtime.
...
```typescript
// Simple text message
smolt.sendUserMessage("What is 2+2?");
...
smolt.sendUserMessage("Focus on error handling", { deliverAs: "steer" });
smolt.sendUserMessage("And then summarize", { deliverAs: "followUp" });

// Opt in to extension command dispatch and skill/prompt template expansion
smolt.sendUserMessage("/review src/index.ts", { expandPromptTemplates: true });
```
...
```typescript
smolt.appendEntry("my-state", { count: 42 });
...
```typescript
const name = smolt.getSessionName();
if (name) {
...
```typescript
// Set a label
smolt.setLabel(entryId, "checkpoint-before-refactor");

// Clear a label
smolt.setLabel(entryId, undefined);
...
```typescript
import type { AutocompleteItem } from "@smolt/tui";

smolt.registerCommand("deploy", {
...
```typescript
const commands = smolt.getCommands();
...
```typescript
smolt.registerMarkdownTransformer((markdown, { messageType, isStreaming }) => {
...
smolt.registerEntryRenderer("status-card", (entry, { expanded }, theme) => {
...
```typescript
smolt.registerShortcut("ctrl+shift+p", {
...
```typescript
const result = await smolt.exec("git", ["status"], { signal, timeout: 5000 });
...
```typescript
const active = smolt.getActiveTools(); // ["read", "bash", ...]
const all = smolt.getAllTools();
// all = [{
//   name: "read",
...
smolt.setActiveTools([...new Set([...active, "my_custom_tool"])]); // Keep current tools and enable my_custom_tool
smolt.setActiveTools(["read", "bash"]); // Switch to read-only
```

`smolt.getAllTools()` returns `name`, `description`, `parameters`, `promptGuidelines`, and `sourceInfo`.
...
if (model) {
  const success = await smolt.setModel(model);
  if (!success) {
...
```typescript
const current = smolt.getThinkingLevel();  // "off" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max"
...
Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `@smolt/ai`. The provider becomes the composition base and `models.json` overrides still apply above it.
...
smolt.registerProvider(provider);

// Register a new provider with custom models
smolt.registerProvider("my-proxy", {
  name: "My Proxy",
...
smolt.registerProvider("llama.cpp", {
  baseUrl: "http://localhost:8080/v1",
  apiKey: "local",
  api: "openai-completions",
...
smolt.registerProvider("anthropic", {
  baseUrl: "https://proxy.example.com"
});

// Register provider with OAuth support for /login
smolt.registerProvider("corporate-ai", {
...
The object form accepts a complete smolt-ai `Provider`, including native `auth`, `getModels`, `refreshModels`, `filterModels`, `stream`, and `streamSimple` behavior.
...
```typescript
smolt.registerCommand("my-setup-teardown", {
  description: "Remove the custom proxy provider",
  handler: async (_args, _ctx) => {
    smolt.unregisterProvider("my-proxy");
  },
});
```
...
Register tools the LLM can call via `smolt.registerTool()`. Tools appear in the system prompt and can have custom rendering.

Use `promptSnippet` for a short one-line entry in the `Available tools` section in the default system prompt. If omitted, custom tools are left out of that section.

Use `promptGuidelines` to add tool-specific bullets to the default system prompt `Guidelines` section. These bullets are included only while the tool is active (for example, after `smolt.setActiveTools([...])`).
...
```typescript
import { withFileMutationQueue } from "smolt";
...
smolt.registerTool({
  ...
  async execute(toolCallId, params, signal, onUpdate, ctx) {
    ...
    // Run commands via smolt.exec (captured from extension closure)
    const result = await smolt.exec("some-command", [], { signal });
...
**Usage accounting:** If a tool makes nested LLM calls, return their combined `Usage` as `usage`. Smolt persists it on the tool result and includes it in footer, `/session`, and RPC session totals. `tool_result` handlers can inspect or replace this value.
...
smolt -e ./tool-override.ts
```

Alternatively, use `--no-builtin-tools` to start without any built-in tools while keeping extension tools enabled:
```bash
# No built-in tools, only extension tools
smolt --no-builtin-tools -e ./my-extension.ts
```
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "smolt";
...
`createBashTool()` and `createPowerShellTool()` expose the current session to commands through `SMOLT_SESSION_ID`, `SMOLT_SESSION_FILE`, `SMOLT_PROVIDER`, `SMOLT_MODEL`, and `SMOLT_REASONING_LEVEL`. Injection happens before `spawnHook`, so hooks receive these values in `env` and preserve them when they spread the existing environment as above. Set `exposeSessionEnvironment: false` to disable them:
...
export default function (smolt: ExtensionAPI) {
  ...
  smolt.registerTool({ name: "db_connect", ... });
  smolt.registerTool({ name: "db_query", ... });
  smolt.registerTool({ name: "db_close", ... });
...
Extensions can register many tools while keeping only a small initial set active. A tool can then add more tools with `smolt.setActiveTools()` during execution. Smolt detects purely additive changes, records the newly available tool names on that tool result, and applies the updated active set before the next model request.
...
1. Register every tool with `smolt.registerTool()` so it appears in `smolt.getAllTools()`.
2. Keep loader tools, such as `search_tools`, active and leave searchable tools inactive.
3. During loader execution, call `smolt.setActiveTools([...currentTools, ...matchingTools])`. The change must be additive: do not remove currently active tools in the same call.
4. Smolt records which tools were added on the loader's tool result.
5. Before the next model response, Smolt exposes the added definitions using native deferred loading when supported, or the normal active tool list otherwise.

You do not need to return provider-specific tool references or mark the loader as a special search tool. The active-tool change is the signal. Names passed to `smolt.setActiveTools()` must already be registered; unknown names are ignored.
...
- **OpenAI**
  ...
  - **Native representation:** Smolt adds completed client `tool_search_call` and `tool_search_output` items at the load point.
...
For all other models and providers, dynamic activation still works: Smolt sends the complete current active tool list normally on the next request. The model can call the newly activated tools, but adding their definitions may invalidate the provider's cached prompt prefix.

Smolt also uses this safe fallback when the active set is not purely additive, such as replacing one group of tools with another. Tool removals therefore work, but they do not use deferred loading.
...
export default function (smolt: ExtensionAPI) {
  ...
  smolt.registerTool({
    ...
    async execute(_toolCallId, params) {
      ...
      const matches = smolt.getAllTools()
      ...
      smolt.setActiveTools([...new Set([...active, ...added])]);
  ...
  smolt.on("session_start", () => {
    ...
    const initialTools = smolt.getActiveTools().filter(
      (name) => !SEARCHABLE_TOOL_NAMES.has(name),
    );
    smolt.setActiveTools([...new Set([...initialTools, "search_tools"])]);
  });
}
```
...
// Terminal title
ctx.ui.setTitle("smolt - my-project");
...
```typescript
import { Text, Component } from "@smolt/tui";
...
import { matchesKey } from "@smolt/tui";
...
smolt.registerMessageRenderer("my-extension", (message, options, theme) => {
...
Messages are sent via `smolt.sendMessage()`:
...
```typescript
smolt.registerEntryRenderer("my-card", (entry, options, theme) => {
  return new Text(theme.fg("accent", JSON.stringify(entry.data)));
});

smolt.appendEntry("my-card", { status: "done" });
```
...
```typescript
import { highlightCode, getLanguageFromPath } from "smolt";
...
| `event-bus.ts` | Inter-extension events | `smolt.events` |
| **Session Metadata** |||
