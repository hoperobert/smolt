...
Sessions are saved automatically to `~/.smolt/agent/sessions/`, organized by working directory.

```bash
smolt -c                  # Continue most recent session
smolt -r                  # Browse and select a session
smolt --no-session        # Ephemeral mode; do not save
smolt --name "my task"    # Set session display name at startup
smolt --session <path|id> # Use a specific session file or session ID
smolt --fork <path|id>    # Fork a session into a new session file
```
...
Smolt loads `AGENTS.md` or `CLAUDE.md` at startup from:

- `~/.smolt/agent/AGENTS.md` for global instructions
- parent directories, walking up from the current working directory
- the current directory

If a directory contains `AGENTS.override.md`, Smolt loads it instead of `AGENTS.md` or `CLAUDE.md` from that directory. Context files from other directories still layer normally.
...
- `.smolt/SYSTEM.md` for a project
- `~/.smolt/agent/SYSTEM.md` globally
...
If no extension or saved decision applies, `defaultProjectTrust` controls the fallback behavior. Set it to `"ask"`, `"always"`, or `"never"` in `~/.smolt/agent/settings.json`, or change it with `/settings`.
...
```bash
smolt [options] [--] [@files...] [messages...]
```

### Package Commands

```bash
smolt install <source> [-l]     # Install package, -l for project-local
smolt remove <source> [-l]      # Remove package
smolt uninstall <source> [-l]   # Alias for remove
smolt update [source|self|smolt]   # Update smolt only, or one package source
smolt update --all              # Update smolt and packages; reconcile pinned git refs
smolt update --extensions       # Update packages only; reconcile pinned git refs
smolt update --models           # Refresh model catalogs only
smolt update --self             # Update smolt only
smolt update --extension <src>  # Update one package
smolt list                      # List installed packages
...
```bash
smolt --no-extensions -e ./my-extension.ts
```
...
```bash
smolt @prompt.md "Answer this"
smolt -p @screenshot.png "What's in this image?"
smolt @code.ts @test.ts "Review these files"
```
...
smolt "List all .ts files in src/"

# Non-interactive
smolt -p "Summarize this codebase"

# Prompt beginning with a dash
smolt -p -- "- Summarize these points"

# Non-interactive with piped stdin
cat README.md | smolt -p "Summarize this text"

# Named one-shot session
smolt --name "release audit" -p "Audit this repository"

# Different model
smolt --provider openai --model gpt-4o "Help me refactor"

# Model with provider prefix
smolt --model openai/gpt-4o "Help me refactor"

# Model with thinking level shorthand
smolt --model sonnet:high "Solve this complex problem"

# Limit model cycling
smolt --models "claude-*,gpt-4o"

# Read-only mode
smolt --tools read,grep,find,ls -p "Review the code"

# Disable one extension or built-in tool while keeping the rest available
smolt --exclude-tools ask_question
```

## Design Principles

Smolt keeps the core small and pushes workflow-specific behavior into extensions, skills, prompt templates, and packages.

It intentionally does not include built-in MCP, sub-agents, permission popups, plan mode, to-dos, or background bash. You can build or install those workflows as extensions or packages, or use external tools such as containers and tmux.

For the full rationale, read the [blog post](https://mariozechner.at/posts/2025-11-30-smolt-coding-agent/).
