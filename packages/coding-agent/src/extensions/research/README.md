...
Send a team of investigators after a subject and get back the answer, with the evidence: findings graded by confidence and carrying their sources, a map of the questions the subject decomposed into, and a synthesized report that leads with the answer. `/research <subject>` investigates the subject **in the session you are in** — nothing is spawned; state a count or a model (`/research 3 researchers into X`, `/research 2 researchers using opencode minimax-m3 into X`) and the work is handed to background researchers instead, exactly as before.

A run is the same in both shapes: a record on disk, findings, the question map, and a report. The difference is who investigates. On its own the session climbs the same ladder a researcher does — it registers `search`, `fetch` and `browse` on first use — so a default `/research` costs one context instead of a fleet's.

A `research` run with a team is built on the same foundation as `/battletest` and looks and behaves the same way: a live roster in the footer and widget, background agent sessions with their own browsers and diaries, a supervising parent that waits the run out, triages as it goes, rules on clearance requests, and synthesizes at the end. Where a tester files tickets, a researcher files **findings**; where battletest's cross-run ledger remembers problems, research keeps a **question map** — the part of wayfinder worth keeping.
...
`/research <subject>` with no count and no model deals no team at all: the run is created with an empty roster and the session investigates it itself (the footer reads `research: solo`). State a count — `/research 3 researchers into <subject>` — or a model, and the team is dealt from the deck and spawned as background sessions; counted and modelled runs are the only ones that spend on child contexts. When a team is dispatched, the angles are non-deterministic: **angles are dealt without replacement** — source-diver, observer, network-sleuth, documentarian, historian, community-listener, comparator, verifier, experimenter, cartographer — and **traits are rolled per researcher**: tenacity (dogged / relentless / obsessive, which also sets the action budget: ~40 / 60 / 80), rigor (accepting / careful / forensic: how much proof a finding needs), and scope (narrow / balanced / wide). A team named by hand — the research tool's `start` with `angles` — may narrow any angle with a focus after a colon (`network-sleuth: the checkout flow`) and seed the question map with up to six sharp sub-questions. `using <provider> <model>` puts every researcher on a named model, exactly as battletest parses it.
...
- **A short brief and a small team.** The brief is about 2.2k tokens with every rule kept (it was 2.9k), and rides on every turn; a team named by hand is capped at three angles, because team size is the wave's cost multiplier. And the default spawns no team at all: `/research <subject>` costs the one context you are already paying for.
...
- **`/research` command** — `<subject>` starts a run this session investigates itself; `<count> researchers [using model] into <subject>` deals a background team instead; `status`, `stop`, `resume [run]`, `continue [run]`, `report [run]`, `list`.
